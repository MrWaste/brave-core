...
class RewardsContributionBrowserTest
 ...
 public:
  ...
  ~RewardsContributionBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(
  ...
  ASSERT_EQ(contribution_->GetMultipleTipStatus(), ledger::Result::LEDGER_OK);
...
IN_PROC_BROWSER_TEST_F(
  ...
  contribution_->VerifyTip(total_amount, should_contribute, false, true);
...
IN_PROC_BROWSER_TEST_F(RewardsContributionBrowserTest, TipConnectedPublisherAnon) {
  // Enable Rewards
  rewards_browsertest_helper::EnableRewards(browser());
  contribution_->AddBalance(promotion_->ClaimPromotionViaCode());
  ...
  contribution_->TipViaCode(
      ...
      should_contribute);
  contribution_->VerifyTip(amount, should_contribute, false, true);
}
...
IN_PROC_BROWSER_TEST_F(RewardsContributionBrowserTest, RecurringTipNonIntegralAmount) {
  rewards_browsertest_helper::EnableRewards(browser());
  contribution_->AddBalance(promotion_->ClaimPromotionViaCode());
